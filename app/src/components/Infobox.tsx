...
export default function Infobox({ infobox }: Props) {
  ...
  return (
    <aside className="border border-ink-300 dark:border-ink-700 p-5 mb-6 bg-white/40 dark:bg-ink-900/40">
      {infobox.infobox && (
        <h3 className="font-serif text-lg text-ink-900 dark:text-ink-100 mb-2">
          {infobox.infobox}
        </h3>
      )}
      {infobox.content && (
        <p className="font-serif text-sm leading-relaxed text-ink-700 dark:text-ink-300 mb-3 line-clamp-6">
          {infobox.content}
        </p>
      )}
